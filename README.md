🌐 Available in English | Español

# RemindPay 💸

RemindPay es una aplicación Android que te ayuda a gestionar suscripciones, pagos recurrentes y deudas en un solo lugar.

Permite a los usuarios monitorear gastos mensuales, recibir recordatorios de pagos próximos y mantener un historial de transacciones para mantenerse organizados financieramente.

Perfecta para gestionar:
- Dinero que prestaste a amigos
- Pagos que debes
- Suscripciones mensuales
- Compras a crédito y pagos en cuotas

Lleva el control de deudas, préstamos y suscripciones en un solo lugar — nunca olvides a quién le debes o quién te debe.
---

## 📱 Características

- Recordatorios de pago (notificaciones)
- Seguimiento de suscripciones y pagos recurrentes
- Gestión de deudas y pagos en cuotas
- Resumen mensual total para suscripciones
- Historial de pagos
- Soporte multi-idioma (Inglés / Español)
- Soporte multi-moneda (USD + NIO)
- Soporte para modo oscuro y claro

---

## 🚀 Descargar

Descarga la última versión del APK desde la sección de **Releases**.

---

## 📸 Capturas de pantalla

<img width="1080" height="2400" alt="image" src="https://github.com/user-attachments/assets/44ec44fc-8e82-4fa4-acd2-58589a4ac05e" />
<img width="1080" height="2400" alt="image" src="https://github.com/user-attachments/assets/6c26edcc-6605-4858-b0ed-ec17bc15a3d2" />
<img width="1080" height="2400" alt="image" src="https://github.com/user-attachments/assets/64599825-ee91-408a-80e0-67ff3226bc9f" />
<img width="1080" height="2400" alt="image" src="https://github.com/user-attachments/assets/cf8df05c-8e21-4ef8-96aa-f1f0162a40e1" />
<img width="1079" height="294" alt="image" src="https://github.com/user-attachments/assets/a9eb4948-e2f7-45c1-b5ed-495f6f8478c0" />
<img width="1380" height="752" alt="RemindPay Debt" src="https://github.com/user-attachments/assets/93c99b08-b9e7-4bbf-b3a8-f2c0151d2b77" />

---

## 🛠️ Desarrollado con

- AndroidStudio (Java/Kotlin)

---

## 🔒 Datos y Privacidad

Todos los datos se almacenan localmente en el dispositivo.  
No se recopila ni comparte información personal.

---

## 🔮 Mejoras futuras

- Respaldo en la nube (Google Drive / Firebase)
- Se añadirá soporte para monedas según la región del dispositivo
- ¡Abierto a feedback o sugerencias!

---

## 👨‍💻 Autor

David Schoures

---

## 📌 Versión

Versión actual: **v1.1**

------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
# ENGLISH

# RemindPay 💸

RemindPay is an Android app that helps you track subscriptions, recurring payments, and debts in one place.

It allows users to monitor monthly expenses, receive reminders for upcoming payments, and keep a history of transactions to stay financially organized.

Perfect for managing:
- Money you lent to friends
- Payments you owe
- Monthly subscriptions
- Installments and credit purchases

Track debts, loans, and subscriptions in one place — never forget who you owe or who owes you.
---

## 📱 Features

-  Payment reminders (notifications)
-  Subscription & recurring payment tracking
-  Debt & installment management
-  Monthly total overview for subscriptions.
-  Payment history tracking
-  Multi-language support (English / Spanish)
-  Multi-currency support (USD + NIO)
-  Dark and Light theme support

---

## 🚀 Download

 Download the latest APK from the **Releases** section.

---

## 📸 Screenshots


<img width="1080" height="2400" alt="image" src="https://github.com/user-attachments/assets/44ec44fc-8e82-4fa4-acd2-58589a4ac05e" />
<img width="1080" height="2400" alt="image" src="https://github.com/user-attachments/assets/6c26edcc-6605-4858-b0ed-ec17bc15a3d2" />
<img width="1080" height="2400" alt="image" src="https://github.com/user-attachments/assets/64599825-ee91-408a-80e0-67ff3226bc9f" />
<img width="1080" height="2400" alt="image" src="https://github.com/user-attachments/assets/cf8df05c-8e21-4ef8-96aa-f1f0162a40e1" />
<img width="1079" height="294" alt="image" src="https://github.com/user-attachments/assets/a9eb4948-e2f7-45c1-b5ed-495f6f8478c0" />
<img width="1380" height="752" alt="RemindPay Debt" src="https://github.com/user-attachments/assets/93c99b08-b9e7-4bbf-b3a8-f2c0151d2b77" />


---

## 🛠️ Built With

- AndroidStudio (Java/Kotlin)

---

## 🔒 Data & Privacy

All data is stored locally on the device.  
No personal data is collected or shared.

---

## 🔮 Future Improvements

-  Cloud backup (Google Drive / Firebase)
-  Support for currencies based on device's region will be added
-  Open to feedback or suggestions!


---

## 👨‍💻 Author

David Schoures

---

## 📌 Version

Current version: **v1.1**
